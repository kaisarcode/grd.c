## Makefile
## Summary: Cross-compilation builder for grd.
##
## Author:  KaisarCode
...
NATIVE_EXE_EXT :=
NATIVE_SHARED_NAME := libgrd.so
NATIVE_IMPORT_LIBRARY :=

ifeq ($(NATIVE_PLATFORM),windows)
NATIVE_EXE_EXT := .exe
NATIVE_SHARED_NAME := libgrd.dll
NATIVE_IMPORT_LIBRARY := -DGRD_TEST_IMPORT_LIBRARY=$(CURDIR)/$(BIN_DIR)/$(NATIVE_TARGET)/libgrd.dll.a
endif
...
define linux_target
	...
	@if [ ! -f $(BUILD_DIR)/$(subst /,-,$(1))-linux/CMakeCache.txt ]; then \
		cmake -S . -B $(BUILD_DIR)/$(subst /,-,$(1))-linux \
			...
			-DGRD_BUILD_VERSION=$(BUILD_VERSION) \
			-DCMAKE_C_COMPILER=$(2) \
...
define windows_target
	...
	$(call cmake_build,$(BUILD_DIR)/$(1)-windows,cmake -S . -B $(BUILD_DIR)/$(1)-windows -DCMAKE_BUILD_TYPE=Release -DCMAKE_SYSTEM_NAME=Windows -DGRD_BUILD_VERSION=$$ver -DCMAKE_C_COMPILER=$(2) -DCMAKE_RUNTIME_OUTPUT_DIRECTORY=$(CURDIR)/$(BUILD_DIR)/$(1)-windows/out -DCMAKE_ARCHIVE_OUTPUT_DIRECTORY=$(CURDIR)/$(BIN_DIR)/$(1)/windows -DCMAKE_LIBRARY_OUTPUT_DIRECTORY=$(CURDIR)/$(BIN_DIR)/$(1)/windows -G Ninja -Wno-dev > /dev/null)
	@cp $(BUILD_DIR)/$(1)-windows/out/grd.exe $(BIN_DIR)/$(1)/windows/grd.exe
	@cp $(BUILD_DIR)/$(1)-windows/out/libgrd.dll $(BIN_DIR)/$(1)/windows/libgrd.dll
	@echo "OK $(1)/windows"
endef
...
test:
	@if [ -n "$(filter wine,$(MAKECMDGOALS))" ]; then \
		...
		if [ ! -f $(BIN_DIR)/x86_64/windows/libgrd.dll ] || [ ! -f $(BIN_DIR)/x86_64/windows/libgrd.dll.a ] || [ ! -f $(BIN_DIR)/x86_64/windows/grd.exe ]; then \
		...
		if [ ! -f $(BUILD_DIR)/test-wine/CMakeCache.txt ]; then \
			cmake -S . -B $(BUILD_DIR)/test-wine \
				...
				-DGRD_BUILD_TESTS=ON \
				-DGRD_TEST_SHARED_LIBRARY=$(CURDIR)/$(BIN_DIR)/x86_64/windows/libgrd.dll \
				-DGRD_TEST_IMPORT_LIBRARY=$(CURDIR)/$(BIN_DIR)/x86_64/windows/libgrd.dll.a \
				-DGRD_TEST_CLI=$(CURDIR)/$(BIN_DIR)/x86_64/windows/grd.exe \
				-DCMAKE_CROSSCOMPILING_EMULATOR=$(WINE) \
				-G Ninja -Wno-dev > /dev/null; \
		fi; \
		cmake --build $(BUILD_DIR)/test-wine --target grd_contract_test || exit 1; \
		...
		if [ ! -f $(BIN_DIR)/$(NATIVE_TARGET)/$(NATIVE_SHARED_NAME) ] || [ ! -f $(BIN_DIR)/$(NATIVE_TARGET)/grd$(NATIVE_EXE_EXT) ]; then \
		...
		if [ ! -f $(BUILD_DIR)/test/CMakeCache.txt ]; then \
			cmake -S . -B $(BUILD_DIR)/test \
				-DCMAKE_BUILD_TYPE=Release \
				-DGRD_BUILD_TESTS=ON \
				-DGRD_TEST_SHARED_LIBRARY=$(CURDIR)/$(BIN_DIR)/$(NATIVE_TARGET)/$(NATIVE_SHARED_NAME) \
				-DGRD_TEST_CLI=$(CURDIR)/$(BIN_DIR)/$(NATIVE_TARGET)/grd$(NATIVE_EXE_EXT) \
				$(NATIVE_IMPORT_LIBRARY) \
				-G Ninja -Wno-dev > /dev/null; \
		fi; \
		cmake --build $(BUILD_DIR)/test --target grd_contract_test || exit 1; \
